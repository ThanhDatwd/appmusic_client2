...
import { AiOutlineYoutube, AiTwotoneHeart, AiOutlineHeart } from 'react-icons/ai'
...
const Media = ({ check=true, listChecked = [], handleCheck = false,
    option = true, rank, prevRank = 2, currentRank = 1, column=false,
    index, song = {}, songs = [], albumId }) => {
    const [isFavorite, setIsFavorite] = React.useState(false)
    const [isChecking, setIsChecking] = React.useState(true)
    // const {listChecked,setListChecked}=React.useContext(albumContext)
                ...
                <>
                    ...
                    <div className="media__right">
                        <div className='media__option'>
                            ...
                            {isFavorite ? <AiTwotoneHeart onClick={() => handleChangeFavoriteSong()} className='media__option-icon add' title='Xóa khỏi thư viện' /> :
                                <AiOutlineHeart onClick={() => handleChangeFavoriteSong()} className='media__option-icon add' title='Thêm vào thư viện' />
                            }
